...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  // Helper to get the ordered stream
  ...
  Future<void> _markAllAsReadHandler() async {
    // 1. Fetch the list of unread documents
    final snapshot = await _inboxCollection?.where('isRead', isEqualTo: false).get();
    final unreadDocs = snapshot?.docs ?? [];

    if (unreadDocs.isEmpty || _inboxCollection == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                return IconButton(
                  ...
                  onPressed: unreadCount > 0 ? _markAllAsReadHandler : null,
